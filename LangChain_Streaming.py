...
class Chatbot():
    ...
    def __init__(self, data_file_path, chat_history, openai_api_key="", pinecone_api_Key="", pinecone_environment=""):
        """
    ...
    def initialize(self):
        ...
        for chat_history in self.chat_history:
            self.memory.save_context({"input": chat_history[0]}, {"output": chat_history[1]})
        self.qa = ConversationalRetrievalChain.from_llm(OpenAI(temperature=0), retriever=self.vectorstore.as_retriever(), memory=self.memory)
    ...
    def filter_Chabot(self, question, existing_answer):
        ...
        You must treat the existing_answer as your own generated answer and avoid acknowledging that it was provided or based on or referred to any other sources. 
        
        You should make it clear that the existing_answer was generated by you as a Axie Infinity chatbot. 
        
        You must treat the existing_answer as your own generated answer and cannot acknowledge that the answer was provided by the existing_answer or any other sources in the answer.
        """
